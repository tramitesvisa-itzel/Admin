...
async function loadFormData() {
    try {
        ...
        querySnapshot.forEach(async (doc) => {
            ...
            row.insertCell(14).textContent = data.persona_con_quien_viaja || '';


            // Datos Personales
            row.insertCell(15).textContent = data.apellido_paterno || '';
            row.insertCell(16).textContent = data.apellido_materno || '';
            row.insertCell(17).textContent = data.nombres || '';
            row.insertCell(18).textContent = data.fecha_nacimiento || '';
            row.insertCell(19).textContent = data.sexo || '';
            row.insertCell(20).textContent = data.lugar_nacimiento || '';
            // Estado Civil
            row.insertCell(21).textContent = data.estado_civil || '';

            // Datos de Cónyuge (solo si está visible)
            row.insertCell(22).textContent = data.nombre_completo_conyuge || '';
            row.insertCell(23).textContent = data.fecha_nacimiento_conyuge || '';
            row.insertCell(24).textContent = data.lugar_nacimiento_conyuge || '';
            
            // Datos del Matrimonio Anterior (solo si está visible)
            row.insertCell(25).textContent = data.nombre_completo_expareja || '';
            row.insertCell(26).textContent = data.fecha_union_expareja || '';
            row.insertCell(27).textContent = data.lugar_nacimiento_expareja || '';
            row.insertCell(28).textContent = data.motivo_separacion_expareja || '';
            row.insertCell(29).textContent = data.fecha_divorcio_expareja || '';
            row.insertCell(30).textContent = data.pais_termino_matrimonio || '';

            // Datos del Cónyuge Union Libre (solo si está visible)
            row.insertCell(31).textContent = data.nombre_completo_conyuge_union_libre || '';
            row.insertCell(32).textContent = data.fecha_nacimiento_conyuge_union_libre || '';
            row.insertCell(33).textContent = data.lugar_nacimiento_conyuge_union_libre || '';

            // Escolaridad
            row.insertCell(34).textContent = data.escolaridad || '';
            row.insertCell(35).textContent = data.nombre_escuela || '';
            row.insertCell(36).textContent = data.direccion_escuela || '';
            row.insertCell(37).textContent = data.fecha_inicio_escuela || '';
            row.insertCell(38).textContent = data.fecha_final_escuela || '';


            // Empleo Actual
            // Información Laboral
            row.insertCell(39).textContent = data.ocupacion_actual_empleo || '';
            row.insertCell(40).textContent = data.nombre_empresa_actual || '';
            row.insertCell(41).textContent = data.domicilio_empresas_actual || '';
            row.insertCell(42).textContent = data.fecha_inicio_empleo_actual || '';
            row.insertCell(43).textContent = data.telefono_empres_actual || '';
            row.insertCell(44).textContent = data.salario_ensual_empleo_actual || '';
            row.insertCell(45).textContent = data.descripcion_actividades_a_su_cargo || '';

 
            // Información Laboral Anterior
            row.insertCell(46).textContent = data.nombre_empresa_anterior || '';
            row.insertCell(47).textContent = data.domicilio_empresas_anterior || '';
            row.insertCell(48).textContent = data.fecha_inicio_empleo_anterior || '';
            row.insertCell(49).textContent = data.fecha_fin_empleo_anterior || '';
            row.insertCell(50).textContent = data.descripcion_actividades_empleo_anterior || '';
            row.insertCell(51).textContent = data.nombre_jefe_inmediato || '';
            row.insertCell(52).textContent = data.telefono_jefe_inmediato || '';

            // Datos del Padre y Madre
            row.insertCell(53).textContent = data.nombre_completo_padre || '';
            row.insertCell(54).textContent = data.fecha_nacimiento_padre || '';
            row.insertCell(55).textContent = data.padre_vive_estados_unidos|| '';
            row.insertCell(56).textContent = data.nombre_completo_madre || '';
            row.insertCell(57).textContent = data.fecha_nacimiento_madre || '';
            row.insertCell(58).textContent = data.madre_vive_estados_unidos || '';

            // Redes Sociales y Familiares
            row.insertCell(59).textContent = data.facebook || '';
            row.insertCell(60).textContent = data.instagram || '';


            // Información de Visas y Viajes
            row.insertCell(61).textContent = data.tiene_familiares_estados_unidos || '';
            row.insertCell(62).textContent = data.nombre_completo_familiar_cercano || '';
            row.insertCell(63).textContent = data.numero_telefonico_familiar_cercano || '';
            row.insertCell(64).textContent = data.domicilio_familiar_cercano || '';
            row.insertCell(65).textContent = data.tramitaron_visa_antes || '';
            row.insertCell(66).textContent = data.redacta_procedimiento_realizado || '';
            row.insertCell(67).textContent = data.han_visitado_estados_unidos || '';
            row.insertCell(68).textContent = data.fecha_visita_estados_unidos || '';
            row.insertCell(69).textContent = data.han_viajado_extranjero_últimos_cinco_años || '';
            row.insertCell(70).textContent = data.pais || '';
            row.insertCell(71).textContent = data.domina_ingles || '';
